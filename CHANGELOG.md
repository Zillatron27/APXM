...
- **Ship unload** (#25) — unload a docked ship's hold from the ship sheet; cargo bars update from the game's own data. Gated buttons state the reason ("Hold is empty" / "In transit — dock to unload")
- **Refuel** — SF and FF refuel from the ship sheet, sourced from the largest local stock; amount = tank deficit capped by stock
- **Load cargo** (#87) — picker with ALL/CONS/OTHER filters, additive quick-amount chips, and live per-row capacity that shrinks as you select; over-capacity is unreachable
- **Send ship** — pick a destination, review APEX's computed route (duration, distance, fees, fuel, damage), set reactor/fuel usage, route preference and options, then SEND. The ship's SFC form is driven off-screen; the SEND tap is the commit
  ...
  - STL fuel use over 50% of the tank shows the consumption figure, fuel readout, and active chip in red (not enough fuel for a return leg)
- **Buffer-card cleanup** (#84) — every card APXM creates while driving an action or a base refresh is deleted on the way out; a card you created yourself with the same command survives. Dev builds add a Buffer Cards sweep tool in Settings for existing cards
...
- Tests: 764
- Ship actions follow the action-authorisation rule: every commit is a deliberate user tap; APXM never sends messages to the game server
